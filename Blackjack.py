...
#  Student's Name: Kenneth Bowen	

#  Student's UT EID: keb2864	
...
class Deck (object):
  def __init__ (self):
    self.deck = []
    #for i in range(num_decks):#(add num_decks as a parameter)
...
class Blackjack (object):
  ...
  def play (self):
    ...
    for i in range (self.numPlayers):
      ...
      #  self.deck.shuffle()
      #self.Players.append (Player([self.deck.deal(), self.deck.deal()]))
      print ('Player ' + str(i + 1) + ': ' + str(self.Players[i]) + " - "+str(self.Players[i].getPoints()) + " points")
...
def main ():
  ...
  game.play()
  #TAKE OUT BEFORE SUBMITTING
  #while input('keep playing? [y / n]') not in ('y','Y'):
  # game.play()
